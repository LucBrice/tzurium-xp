    ...
    *,
    ...
    for index, raw_review in enumerate(reviews):
        ...
        if review.get("state") not in SUBMITTED_REVIEW_STATES:
            continue
        # GitHub returns commit: null when the reviewed commit is no longer
        # reachable (force-push). Such a review cannot be bound to head, so
        # skip it rather than refusing the whole payload -- same reasoning
        # already applied to a null author above.
    ...
    if expected_head is None:
        # The type annotation alone does not enforce this at runtime: a
        # caller that omits expected_head would otherwise get PASS on a
        # payload never actually bound to a specific, independently-known
        # head. Every other rejection above is allowed to fire without it
        # (they are refusals regardless), but PASS itself always requires it.
        raise VerdictError("INCOMPLETE: expected_head is required to return PASS")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"CODERABBIT_VERDICT_REFUSED: {exc}", file=sys.stderr)
        return 2
